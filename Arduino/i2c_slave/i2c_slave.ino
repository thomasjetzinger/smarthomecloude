...
float CalculateNtcTemp(long adcValue);
...
void loop()
{
  ...
  Value_Temperature = (short)CalculateNtcTemp(analogRead(Pin_Temperature));
  //(short)((float)(analogRead(Pin_Temperature) * 0.48828125));
...
// NTCLE100E3 - Number 9 (B=3977)
float CalculateNtcTemp(long adcValue)
{
  long rth;
  float temp;

  // Paremters from datasheet
  const float a = 3.354016e-3;
  const float b = 2.569355e-4;
  const float c = 2.626311e-6;
  const float d = 6.383091e-7;

  rth = (adcValue * 1000) / (1024 - adcValue);  // now we have the resistance in ohms

  temp = log(rth / 4700.0);

  // Formula from datasheet
  rth = (1.0 / (a + b * temp + c * temp * temp + d * temp * temp * temp)) - 273.15;   
